...
Usage:
    ...
    python evaluators.py \
        ...
        --output process_mining/results/pattern_eval.json

    # Baseline mode (no patterns)
    python evaluators.py \
        --baseline \
        --validation-data ../../data/pattern_data/validation_pattern_data \
        --output process_mining/results/baseline_eval.json
...
from process_mining.core.classifiers import PatternBasedClassifier, PLATFORM_CONFIGS
...
Examples:
  ...
  python evaluators.py \
      --patterns ../../data/patterns/train_patterns/generic_patterns/issue_type_patterns_summary \
      --validation-data ../../data/pattern_data/validation_pattern_data \
      --output process_mining/results/eval.json \
      --workers 10 \
      --verbose
  ...
  python evaluators.py \
      ...
      --validation-data ../../data/pattern_data/validation_pattern_data \
      --limit 10 \
      --output process_mining/results/baseline_test.json
        """
    )
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("process_mining/results") / f"evaluation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
        help="Output file for evaluation results (default: process_mining/results/evaluation_TIMESTAMP.json)"
    )
