...
# Platform configurations - loaded from environment variables
PLATFORM_CONFIGS = {
    "local": {
        "base_url": os.getenv("LOCAL_BASE_URL", "http://localhost:8000/v1"),
        "api_key": os.getenv("LOCAL_API_KEY", "API_KEY"),
...
class PatternBasedClassifier:
    ...
    def load_patterns(self, patterns_dir: Optional[Path] = None) -> None:
        """
        Load pattern files from directory, auto-detecting issue type from filename.

        Automatically extracts issue type from filename patterns:
        - ISSUE_TYPE_patterns.json -> ISSUE_TYPE
        - rhel_c_ISSUE_TYPE_pattern_summary.json -> ISSUE_TYPE
        - any_prefix_ISSUE_TYPE_any_suffix.json -> ISSUE_TYPE

        Works with ANY issue type without requiring manual configuration.
        ...
        for json_file in patterns_dir.glob('*.json'):
            issue_type = self._extract_issue_type_from_filename(json_file.name)
            if issue_type:
    ...
    def _extract_issue_type_from_filename(self, filename: str) -> Optional[str]:
        """
        Extract issue type from pattern filename.

        Supports multiple naming conventions:
        - ISSUE_TYPE_patterns.json
        - ISSUE_TYPE_iteration_N_patterns.json
        - rhel_c_ISSUE_TYPE_pattern_summary.json
        - any_ISSUE_TYPE_suffix.json

        Args:
            filename: Pattern filename

        Returns:
            Issue type in uppercase or None if not detected
        """
        name = filename.replace('.json', '')

        match = re.match(r'^([A-Z_]+)_(?:iteration_\d+_)?patterns?$', name)
        if match:
            return match.group(1)

        match = re.match(r'^rhel_c_([A-Z_]+)_pattern_summary$', name)
        if match:
            return match.group(1)

        matches = re.findall(r'[A-Z][A-Z_]+[A-Z]', name)
        if matches:
...
**CRITICAL RULE**: Patterns are MANDATORY. You MUST prioritize patterns above all else.
...
3. **Check Reference Patterns FIRST**: Review the patterns provided above and determine if any pattern matches this finding.
   - These patterns were learned from expert-labeled examples
   - They represent validated knowledge about this issue type
   - Pattern matching takes PRIORITY over your independent analysis

4. **Make Your Decision**:
   - **If a pattern applies**: You MUST use the pattern to determine TRUE_POSITIVE or FALSE_POSITIVE. The pattern's guidance is authoritative. Do NOT override it with your own analysis.
   - **If no pattern applies**: Only then perform your own code analysis to determine the classification.

**DOUBLE EMPHASIS**:
- Patterns are learned from ground truth data and are MORE RELIABLE than independent analysis
- When a pattern matches, USE IT - do not second-guess the pattern
- Only fall back to independent analysis when NO pattern is applicable
"""
            else:
                prompt += f"""
## Analysis Plan

**Note**: No pre-learned patterns are available for {issue_type} issues. You should rely on your own security analysis expertise.

1. **Read the Error Trace**: Understand the flow and location of the reported issue.
2. **Read the Source Code Context**: Examine the relevant code attached above.
3. **Analyze the Code**: Apply your knowledge of common {issue_type} vulnerabilities and security best practices to determine if this is a TRUE_POSITIVE (real security vulnerability) or FALSE_POSITIVE (incorrect/benign finding).

Even without patterns, you MUST still make a classification decision based on:
- Your understanding of {issue_type} vulnerabilities
- The code context and data flow analysis
- Common false positive scenarios for this issue type
"""
        else:
            prompt += """
## Analysis Plan
...
Examples:
  ...
  python classifiers.py patterns/ validation.txt

  # Use NVIDIA NIM platform with Qwen3 model
  python classifiers.py patterns/ validation.txt --platform nim
